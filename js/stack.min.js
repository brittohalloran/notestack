...
// NOTESTACK CODE
// http://notestack.me
var notestack=function(){var a=jQuery.noConflict(),s,t,u=function(b){b==="off"?(a("html body").removeClass("fullscreen-mode"),a(".overlay").removeClass("show")):b==="on"?(a("html body").addClass("fullscreen-mode"),a(".overlay").addClass("show")):(a("html body").toggleClass("fullscreen-mode"),a(".overlay").toggleClass("show"))},i=function(){a(".show").not(".settings_pane,.msg").removeClass("show");u("off")},p=function(b,d){a(".status .progress .inner").length===0&&a(".status").html('<div class="progress"><div class="inner"></div></div>');
a(".status .progress .inner").animate({width:b+"%"},250,d)},Q=a.throttle(500,!0,p),w=function(b){b==="clear"?(a(".msg .inner").text(""),a(".msg").removeClass("show")):(a(".msg .inner").text(b),a(".msg").addClass("show"))},R=function(){return a.Deferred(function(b){a.ajax("/sn.php",{type:"POST",data:{action:"tagIndex",email:localStorage.email,token:localStorage.token},timeout:3E3,success:function(a){if(a==="401")window.location="/?login=expired";localStorage.tagIndex=a;b.resolve()},error:function(){a(".status").text("Working offline. Changes will sync when connected.");
a(".status-div").removeClass("loading")}})})},F=function(b){return a.Deferred(function(d){a.ajax("/sn.php",{type:"POST",data:b,timeout:5E3,success:function(c){var e,f,g,h,o;if(c==="401")window.location="/?login=expired";c=a.parseJSON(c);if(localStorage.index){e=a.parseJSON(localStorage.index);for(f=0,g=c.data.length;f<g;f+=1){for(h=0,o=e.data.length;h<o;h+=1)if(e.data[h].key===c.data[f].key){e.data.splice(h,1);break}e.data.push(c.data[f])}e.count=e.data.length}else e=c;localStorage.setItem("mark",
c.mark||"DONE");localStorage.setItem("index",JSON.stringify(e));localStorage.setItem("indexDate",c.time);localStorage.mark==="DONE"?(localStorage.removeItem("mark"),d.resolve()):(b.mark=localStorage.mark,a.when(F(b)).done(function(){d.resolve()}))},error:function(){a(".status").text("Working offline. Changes will sync when connected.");a(".status-div").removeClass("loading")}})}).promise()},l=function(b){return a.Deferred(function(d){var c=a.parseJSON(localStorage.getItem(b));if(c.deleted!==1){a("#list-"+
c.key).length>0?a("#list-"+c.key).removeData().data(c):a("#listnote-template").clone().attr("id","list-"+c.key).addClass("listnote").appendTo(".list").data(c);c=a("#list-"+c.key);c.removeClass("pinned");c.data().systemtags.indexOf("pinned")>-1&&c.addClass("pinned");var e=c.data().content.indexOf("\n");e<0?(c.children(".title").text(c.data().content.substring(0,100)),c.children(".preview").text("")):(c.children(".title").text(c.data().content.substring(0,e)),c.children(".preview").text(c.data().content.substr(e,
100)));d.resolve()}}).promise()},q=function(){a(".listnote").sortElements(function(b,d){if(a(b).hasClass("pinned")){if(!a(d).hasClass("pinned"))return-1}else if(a(d).hasClass("pinned")&&!a(b).hasClass("pinned"))return 1;return parseFloat(a(b).data("modifydate"))<parseFloat(a(d).data("modifydate"))?1:-1});return"sorted"},v=function(){var b=a("#"+a(".selected").data().key),d=b.prevAll(".on").eq(1),c=b.prevAll(".on").eq(0),e=b.nextAll(".on").eq(0),f=b.nextAll(".on").eq(1);b.hasClass("current")||(a(".current").removeClass("current"),
b.addClass("current"));d.hasClass("prev2")||(a(".prev2").removeClass("prev2"),d.addClass("prev2"));c.hasClass("prev1")||(a(".prev1").removeClass("prev1"),c.addClass("prev1"));e.hasClass("next1")||(a(".next1").removeClass("next1"),e.addClass("next1"));f.hasClass("next2")||(a(".next2").removeClass("next2"),f.addClass("next2"))},S=function(a){return a.replace(/\n/g,"<br>").replace(/\r/g,"")},m=function(){var b,d,c,e,f;a(".selected").length!==1&&(a(".selected").removeClass("selected"),a(".listnote:visible:first").addClass("selected"));
c=a.parseJSON(localStorage.tagIndex).tags;e=[];for(b=0,d=c.length;b<d;b+=1)e.push(c[b].name);var g=function(b){var c,d,g;a("#note-template").clone().attr("id",b.key).addClass("note").insertAfter(f);c=a("#"+b.key);c.data("syncnum",b.syncnum);c.children(".textarea").children(".ta-inner").children("textarea").val(b.content);d=b.tags.join(" ");g=c.children(".textarea").children(".tag-area").children("input");g.val(d);g.tagit({singleFieldDelimiter:" ",availableTags:e}).tagit({onTagAdded:function(){x(a(".current").attr("id"))},
onTagRemoved:function(b,c){var e=c.children(".tagit-label").text();x(a(".current").attr("id"),e)}});c.addClass("on");a.inArray("markdown",b.systemtags)>-1&&r(b.key,"on");a.inArray("pinned",b.systemtags)>-1&&c.addClass("pinned");y("#"+b.key+" .textarea textarea")};f=a(".notes-placeholder");a(".listnote").each(function(){var b=a(this).data(),c=a("#"+b.key).length>0,e=c?a("#"+b.key):!1;a(this).css("display")==="none"?c&&e.removeClass("on").removeClass("current").removeClass("prev1").removeClass("prev2").removeClass("next1").removeClass("next2"):
c?(b.syncnum>e.data().syncnum?(e.remove(),g(b),e=a("#"+b.key)):e.addClass("on"),e.insertAfter(f)):(g(b),e=a("#"+b.key));f=e});v()},G=function(b){var d=a.Deferred(),c={action:"sendnote",email:s,token:t,notekey:b.key,notebody:JSON.stringify(b)},e=0;if(b.key.substr(0,9)==="notestack")delete c.notekey,delete b.key,c.notebody=JSON.stringify(b),e=1;a.ajax("/sn.php",{type:"POST",data:c,success:function(c){var g,h,o,c=a.parseJSON(c);if(c.content===void 0)c.content=b.content;if(e===1){g=a.parseJSON(localStorage.index);
for(h=0,o=g.data.length;h<o;h+=1)if(g.data[h].key.substr(0,9)==="notestack"){a("#list-"+g.data[h].key).remove();localStorage.removeItem(g.data[h].key);g.data.splice(h,1);break}localStorage.index=JSON.stringify(g)}localStorage.setItem(c.key,JSON.stringify(c));l(c.key);a("#list-"+c.key).addClass("selected");q();m();d.resolve()},error:function(){}});return d.promise()},H=function(b){return a.Deferred(function(d){a.ajax("/sn.php",{type:"POST",data:{action:"note",email:s,token:t,notekey:b},success:function(b){var e;
e=a.parseJSON(b);localStorage.setItem(e.key,b);l(e.key);d.resolve()},error:function(){}})}).promise()},T=function(){return a.Deferred(function(b){function d(j){var i,n;for(e=g,f=g+j;e<=f;e+=1){Q(50+Math.round(50*(e/c.data.length)));if(!c.data[e]){b.resolve();n=!0;break}localStorage.getItem(c.data[e].key)?(i=a.parseJSON(localStorage.getItem(c.data[e].key)),i.key.substr(0,9)==="notestack"?h.push(G(i)):i.syncnum<c.data[e].syncnum?h.push(H(i.key)):i.syncnum>c.data[e].syncnum?h.push(G(i)):l(i.key)):h.push(H(c.data[e].key));
c.data[e].deleted===0?o+=1:c.data[e].deleted===1&&(k+=1)}if(n===!0)return a(".note-data").text(o+" notes | "),a(".data_notes").text(o),a(".data_deleted").text(k),!1;h.length>0?a.when.apply(null,h).done(function(){g+=j+1;h=[];q();m();d(j)}):(g=g+j+1,h=[],d(j))}var c,e,f,g=0,h=[],o=0,k=0;c=a.parseJSON(localStorage.index);d(20)}).promise()},I=function(a){a=parseFloat(a);return(new Date(a*1E3)).toISOString()},n=function(b){p(5);localStorage.token?a.when(R()).done(function(){var d,c;p(25);b==="full"?c=
d="":localStorage.mark?(d="",c=localStorage.mark):(d=localStorage.indexDate||"",c="");s=localStorage.email;t=localStorage.token;a.when(F({action:"index",email:s,token:t,since:d,mark:c,length:100})).done(function(){p(50);(new Date).getTime();a.when(T()).done(function(){p(100,function(){var b;b=I(localStorage.indexDate);a(".status").html('synced <abbr class="timeago" title="'+b+'"></abbr>');a(".timeago").timeago();a(".status-div").removeClass("loading");q();m()})})})}):window.location="/"},r=function(b,
d,c){b=b||a(".current").attr("id");d===void 0&&(d=a("#"+b).hasClass("markdown-on")?"off":"on");c!=="temp"&&(c="stick");var e=a("#"+b),b=a("#list-"+b),f=b.data(),g=e.children(".textarea");if(d==="on")e.removeClass("markdown-off").addClass("markdown-on"),c=(new Markdown.Converter).makeHtml("# "+g.children(".ta-inner").children("textarea").val()),g.children(".ta-inner").replaceWith('<div class="markdown">'+c+"</div>"),g.children(".markdown").children("h1:first").addClass("first-h1"),g.children(".markdown").children("p").each(function(){a(this).html(S(a(this).html()))}),
a.inArray("markdown",f.systemtags)<0&&(f.systemtags.push("markdown"),f.syncnum+=1,localStorage.setItem(f.key,JSON.stringify(b.data())),l(f.key),n());else if(d==="off")e.removeClass("markdown-on").addClass("markdown-off"),d=f.content,g.children(".markdown").replaceWith('<div class="ta-inner"><textarea>'+d+"</textarea></div>"),y(g.children(".ta-inner").children("textarea")),c==="stick"&&(c=b.data().systemtags.indexOf("markdown"),c>-1&&f.systemtags.splice(c,1),f.syncnum+=1,localStorage.setItem(f.key,
JSON.stringify(b.data())),l(f.key),n())},x=function(b,d){var c,e;c=a("#"+b+" .tag-area input").val().split(" ");e=c.indexOf(d);e!==-1&&c.splice(e,1);e=a.parseJSON(localStorage[b]);if(e.tags!==c)e.tags=c,e.syncnum+=1,localStorage[b]=JSON.stringify(e),l(b),n()},z=function(){var b=a(".search input").val().toLowerCase().split(" ");a(".listnote").each(function(){var d=a(this).data().content.toLowerCase(),c,e;for(c=0,e=b.length;c<e;c+=1)if(d.indexOf(b[c])>-1)a(this).removeClass("search-hide");else{a(this).addClass("search-hide");
break}});a(".selected:not(.search-hide,.tag-hide)").length===0&&(a(".selected").removeClass("selected"),a(".listnote:not(.search-hide,.tag-hide):first").addClass("selected"));m()},y=function(b){a(b).bind("keydown","esc",function(){a(".note-directions").removeClass("show");a(this).blur()});a(b).tabby();a(".maximize").click(function(){u()});a(".tagsinput div input").blur(function(){x(a(".current").attr("id"))})},J=function(){a(".selected").position().top<0?a(".list").scrollTop(a(".list").scrollTop()+
a(".selected").position().top):a(".selected").position().top+a(".listnote").height()>a(".list").height()&&a(".list").scrollTop(a(".list").scrollTop()+a(".selected").position().top-a(".list").height()+a(".listnote").height())},k=function(){a(".selected").nextAll(".listnote:not(.hide):first");var b=a(".selected").nextAll(".listnote:visible:first");b.length>0&&(a(".selected").removeClass("selected"),b.addClass("selected"),a(".prev2").removeClass("prev2"),a(".prev1").addClass("prev2").removeClass("prev1"),
a(".current").addClass("prev1").removeClass("current"),a(".next1").addClass("current").removeClass("next1"),a(".next2").addClass("next1").removeClass("next2"),a(".next1").nextAll(".on").eq(0).addClass("next2"),J())},j=function(){var b=a(".selected").prevAll(".listnote:visible:first");b.length>0&&(a(".selected").removeClass("selected"),b.addClass("selected"),a(".next2").removeClass("next2"),a(".next1").removeClass("next1").addClass("next2"),a(".current").removeClass("current").addClass("next1"),a(".prev1").removeClass("prev1").addClass("current"),
a(".prev2").removeClass("prev2").addClass("prev1"),a(".prev1").prevAll(".on").eq(0).addClass("prev2"),J())},K=function(b){w("Working...");b===void 0&&a(".selected").length>-1&&(b=a(".selected"));b.data().systemtags.indexOf("pinned")>-1?b.data().systemtags.splice(b.data().systemtags.indexOf("pinned"),1):b.data().systemtags.push("pinned");b.data().syncnum=b.data().syncnum+1;localStorage.setItem(b.data().key,JSON.stringify(b.data()));l(b.data().key);q();m();w("clear")},L=function(){var b,d,c;b=(new Date).getTime();
d="notestack"+b;b={key:d,createdate:b/1E3,modifydate:b/1E3,tags:[],systemtags:[],content:"new note"};localStorage.setItem(d,JSON.stringify(b));c=a.parseJSON(localStorage.index);c.data.push(b);localStorage.index=JSON.stringify(c);l(d);q();a(".selected").removeClass("selected");a("#list-"+d).addClass("selected");m();a(".current .textarea textarea").select()},M=function(){a("html body").removeClass("tag-filter");a(".active-tag-filter").remove();a(".listnote").removeClass("tag-hide");a(".selected").removeClass("selected");
a(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");a(".search input").val("");z()},U=function(){a(".version-item").sortElements(function(b,d){return parseFloat(a(b).data("version"))<parseFloat(a(d).data("version"))?1:-1});return"sorted"},V=function(b,d){return a.Deferred(function(c){a.ajax("/sn.php",{type:"POST",data:{action:"noteversion",email:s,token:t,notekey:b,noteversion:d},success:function(e){e=a.parseJSON(e);a("#version-template").clone().attr("id",b+"version"+e.version).addClass("version-item").appendTo(".versions .versions-left").data(e);
a("#"+b+"version"+e.version).children(".version-date").html('<abbr class="timeago" title="'+I(e.versiondate)+'"></abbr>');a(".timeago").timeago();U();c.resolve()},error:function(){return"false"}})}).promise()},W=function(b){a(".version-item").remove();for(var d=[],c=a.parseJSON(localStorage.getItem(b)),e=c.version,f=c.minversion,c=f;c<=e;c+=1)d.push(V(b,c));a.when.apply(null,d).done(function(){a(".version-item").each(function(){if(a(this).data().version>f){var c=a(this).data().version-1,e=a("#"+b+
"version"+c).data().content,d=a(this).data().content,c=new Diff_Match_Patch,e=c.diff_main(e,d);c.diff_cleanupSemantic(e);c=c.diff_prettyHtml(e);a(this).data("diff",c)}else a(this).data().version===f&&a(this).data("diff",a(this).data().content)});a(".versions-right .right-inner").html(a(".version-item:first").data().diff);a(".version-item:first").addClass("version-select")})},N=function(b){a(".versions").hasClass("show")?i():(u("off"),a(".versions").addClass("show"),a(".overlay").addClass("show"),
a(".versions-right .right-inner").html(""),W(b))},X=function(){a(".labels .item").remove();var b=a.parseJSON(localStorage.tagIndex);a(".listnote");var d,c,e,f,g,h=function(){a.inArray(e,a(this).data().tags)>-1&&(a(this).data(),g+=1)};for(d=0,c=b.count;d<c;d+=1)e=b.tags[d].name,f=e.replace("@","").replace(".",""),g=0,a(".listnote").each(h()),a("#label-template").clone().attr("id",f).addClass("item").appendTo(".labels .items"),a("#"+f).children(".name").html("<pre>"+e+"</pre>"),a("#"+f).children(".notecount").text(g+
" notes");a(".labels .item").sortElements(function(b,c){return parseFloat(a(b).children(".notecount").text())<parseFloat(a(c).children(".notecount").text())?1:-1})},A=function(){a(".labels").hasClass("show")?i():(u("off"),X(),a(".labels").addClass("show"),a(".overlay").addClass("show"),a(".labels input").val("").select());return!1},O=function(){var b=a(".labels input").val().toLowerCase();a(".labels .item").each(function(){a(this).children(".name").text().toLowerCase().indexOf(b)>-1?a(this).removeClass("hide"):
a(this).addClass("hide")});a(".label-select").removeClass("label-select");a(".labels .item:not(.hide):first").addClass("label-select")},P=function(b){a(".tag-hide").removeClass("tag-hide");a(".active-tag-filter").remove();b!=="partial"&&(a("html body").removeClass("tag-filter"),a(".selected").removeClass("selected"),a(".listnote:not(.search-hide,.tag-hide):first").addClass("selected"),m())},B=function(b){w("Working...");a(".labels input").val("").blur();i();P("partial");a("html body").addClass("tag-filter");
a("#tag-filter-template").clone().attr("id","").addClass("active-tag-filter").insertBefore(".list");a(".active-tag-filter .inner .tag-filter-label").text(b);a(".listnote").each(function(){a.inArray(b,a(this).data().tags)<=-1&&a(this).addClass("tag-hide")});a(".selected").removeClass("selected");a(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");m();w("clear")},C=function(b){var d=a("html body").attr("class").split(" "),c,e;for(c=0,e=d.length;c<e;c+=1)d[c].substr(0,6)==="theme-"&&
(d[c]="");a(".appearance .item.theme").siblings(".theme").addClass("dim");a("#"+b).removeClass("dim");a("html body").attr("class",d.join(" "));a("html body").addClass(b);localStorage.theme=b},D=function(b){var d=a("html body").attr("class").split(" "),c,e;for(c=0,e=d.length;c<e;c+=1)d[c].substr(0,5)==="font-"&&(d[c]="");a(".appearance .font").siblings(".font").addClass("dim");a("#"+b).removeClass("dim");a("html body").attr("class",d.join(" "));a("html body").addClass(b);localStorage.font=b},E=function(b){var d=
a("html body").attr("class").split(" "),c,e;for(c=0,e=d.length;c<e;c+=1)d[c].substr(0,9)==="fontsize-"&&(d[c]="");a(".appearance .fontsize").siblings(".fontsize").addClass("dim");a("#"+b).removeClass("dim");a("html body").attr("class",d.join(" "));a("html body").addClass(b);localStorage.fontsize=b},Y=function(){var b,d,c;localStorage.theme?(b=localStorage.theme,C(b)):C("theme-moleskine");a(".appearance .item.theme").click(function(){b=a(this).attr("id");a(this).removeClass("dim");C(b)});localStorage.font?
(d=localStorage.font,D(d)):D("font-mono");a(".appearance .font").click(function(){d=a(this).attr("id");D(d)});localStorage.fontsize?(c=localStorage.fontsize,E(c)):E("fontsize-12");a(".appearance .fontsize").click(function(){var b=a(this).attr("id");E(b)});localStorage.tagPref==="no-tags"&&(a("html body").addClass("no-tags"),a(".tag-pref").addClass("dim"));a(".appearance .tag-pref").click(function(){a(this).hasClass("dim")?(a(this).removeClass("dim"),a("html body").removeClass("no-tags"),localStorage.removeItem("tagPref")):
(a(this).addClass("dim"),a("html body").addClass("no-tags"),localStorage.setItem("tagPref","no-tags"))});a(".note .textarea textarea").live("focus",function(){a(".current .note-directions").addClass("show");a("div.current").addClass("highlight")});a(".note .textarea textarea").live("blur",function(){var b=a(this).parent().parent().parent().attr("id"),c=a("#list-"+b).data();a.inArray("markdown",c.systemtags)>-1&&r(b,"on");if(a(this).val()!==c.content)c=a.parseJSON(localStorage.getItem(b)),c.content=
a(this).val(),c.modifydate=(new Date).getTime()/1E3,c.syncnum+=1,localStorage.setItem(b,JSON.stringify(c)),l(c.key),n();a(".current .note-directions").removeClass("show");a(".current").removeClass("highlight")});a(".addnote").click(function(){L()});a(".tags").click(function(){A()});a(".reload").click(function(){n()});a(".close").click(function(){i()});a(".delete").live("click",function(){a(this).parent().children(".message").toggleClass("show").html('Mark this note as deleted?  <a href="#" class="confirmDelete">Delete</a> / <a href="#" class="cancelDelete">Cancel</a>')});
a(".confirmDelete").live("click",function(){var b=a(this).parent().parent().attr("id"),c=a("#list-"+b).data();c.deleted=1;c.modifydate=(new Date).getTime()/1E3;c.syncnum+=1;localStorage.setItem(c.key,JSON.stringify(c));a("#list-"+b).remove();a("#"+b).remove();a(".listnote:first").addClass(".selected");m();return!1});a(".cancelDelete").live("click",function(){a(".message").removeClass("show");return!1});a(".listnote").live("click",function(){var b=a(".selected"),c=a(".listnote:visible").index(b),d=
a(".listnote:visible").index(this),b=Math.abs(d-c),c=c<d?"next":"prev";if(b<=6)if(c==="next")for(d=1;d<=b;d+=1)k();else{if(c==="prev")for(d=1;d<=b;d+=1)j()}else c==="next"?(a(".selected").removeClass("selected"),a(".listnote:visible").eq(d-4).addClass("selected"),v(),k(),k(),k(),k()):c==="prev"&&(a(".selected").removeClass("selected"),a(".listnote:visible").eq(d+4).addClass("selected"),v(),j(),j(),j(),j())});a(".note").live("click",function(){a(".current").removeClass("current");a(this).addClass("current");
a(".selected").removeClass("selected");a("#list-"+a(this).attr("id")).addClass("selected");v()});a(".textarea:not(a)").live("click",function(b){a(b.target).parents(".tag-area").length>0||(r(a(this).parent().attr("id"),"off","temp"),a(this).children(".ta-inner").children("textarea").focus())});a(".markdown a").live("click",function(){a(this).attr("target","_blank")});a(".settings_icon").click(function(){a(".settings").toggleClass("show")});a(".userdata").click(function(){a(".settings").toggleClass("show");
a(".settings_pane").removeClass("show");a(".settings_pane.data").addClass("show")});a(".settings .tabs a").click(function(b){a(".settings_pane").removeClass("show");a(".settings_pane."+a(this).text().replace(" ","_")).addClass("show");b.preventDefault();return!1});a(".versions-button").live("mousedown",function(){var b=a(this).parent().attr("id");N(b)});a(".version-item").live("mousedown",function(){a(".diff-view").hasClass("depressed")?a(".versions-right .right-inner").html(a(this).data().diff):
a(".versions-right .right-inner").html('<textarea disabled="disabled">'+a(this).data().content+"</textarea>");a(".version-select").removeClass("version-select");a(this).addClass("version-select")});a(".versions-toolbar .diff-view,.versions-toolbar .text-view").live("mousedown",function(){a(".versions-toolbar .depressed").removeClass("depressed");a(this).addClass("depressed");a(".version-select").length<1&&a(".version-item:first").addClass(".version-select");a(this).hasClass("diff-view")?a(".versions-right .right-inner").html(a(".version-select").data().diff):
a(this).hasClass("text-view")&&a(".versions-right .right-inner").html('<textarea disabled="disabled">'+a(".version-select").data().content+"</textarea>")});a(".overlay").click(function(){i()});a(".versions-toolbar .revert-version").live("mousedown",function(){if(a(".version-select").length===1){var b=a(".version-select").attr("id").split("version").shift(),c=a(".version-select").data().content;a(".selected").removeClass("selected");a("#list-"+b).addClass("selected");a(".current").removeClass("current");
a("#"+b).addClass("current");v();i();a(".note.current .textarea textarea").val(c).focus()}return!1});a(".clear-tag-filter").live("mouseup",function(){P()});a(".labels .item").live("mouseup",function(){B(a(this).attr("id"))});a(".tagit-label").live("mouseup",function(){B(a(this).text())});a(".markdown-button").live("mouseup",function(){r(a(this).parent().attr("id"))});a(".pin-button").live("mouseup",function(){K()});a(".cleardata").click(function(){localStorage.clear()});a(document).bind("keydown",
"esc",function(){i()});a(document).bind("keydown","j",function(){k()});a(document).bind("keydown","right",function(){k()});a(document).bind("keydown","down",function(){k()});a(document).bind("keydown","k",function(){j()});a(document).bind("keydown","left",function(){j()});a(document).bind("keydown","up",function(){j()});a(document).bind("keydown","return",function(){a(".current").hasClass("markdown-on")&&r(a(".current").attr("id"),"off","temp");a(".note.current .textarea textarea").focus();return!1});
a(document).bind("keydown","tab",function(){a(".note.current .textarea textarea").focus();return!1});a(document).bind("keydown","shift+/",function(){a(".settings").toggleClass("show")});a(document).bind("keydown","/",function(){a(".search input").select();return!1});a(document).bind("keydown","c",function(){L();return!1});a(document).bind("keydown","s",function(){n();return!1});a(document).bind("keydown","p",function(){K(a(".selected"))});a(document).bind("keydown","f",function(){u()});a(document).bind("keydown",
"l",function(){A();return!1});a(document).bind("keydown","t",function(){A();return!1});a(document).bind("keydown","v",function(){N(a(".current").attr("id"));return!1});a(document).bind("keydown","a",function(){M();return!1});a(document).bind("keydown","i",function(){M();return!1});a(document).bind("keydown","m",function(){r();return!1});a(".search input").keyup(a.debounce(250,z));a(".search input").bind("keydown","return",function(){a(this).blur();return!1});a(".search input").bind("keydown","esc",
function(){a(this).val("");z();a(this).blur();return!1});a(".search input").bind("keydown","down",function(){a(this).blur();k();return!1});a(".search input").bind("keydown","up",function(){a(this).blur();j();return!1});a(".labels input").keyup(function(a){a=a.which;a!==38&&a!==40&&a!==13&&O()});a(".labels input").bind("keydown","esc",function(){a(this).val("");a(this).blur();i();return!1});a(".labels input").bind("keydown","down",function(){var b=a(".label-select").nextAll(".item:not(.hide):first");
b.length>0&&(a(".label-select").removeClass("label-select"),b.addClass("label-select"));return!1});a(".labels input").bind("keydown","up",function(){var b=a(".label-select").prevAll(".item:not(.hide):first");b.length>0&&(a(".label-select").removeClass("label-select"),b.addClass("label-select"));return!1});a(".labels input").bind("keydown","return",function(){a(".label-select").length===1?B(a(".label-select .name").text()):a(".labels").effect("shake",{times:3},100,function(){a(".labels input").val("").select();
O()});return!1});y(".note .textarea textarea")};a(function(){Y();if(localStorage.email){a(".username").text(localStorage.email);if(localStorage.index){var b,d,c;p(10);if(localStorage.index){c=a.parseJSON(localStorage.index);for(b=0,d=c.data.length;b<d;b+=1)localStorage.getItem(c.data[b].key)!==null&&l(c.data[b].key)}a(".listnote").length>0&&(q(),m())}(new Date).getTime();n()}else window.location="/"})}();