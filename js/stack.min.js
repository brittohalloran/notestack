...
// NOTESTACK CODE
// http://notestack.me
var notestack=function(){$=jQuery.noConflict();var h=function(){$(".show").not(".settings_pane,.msg").removeClass("show");r("off")},r=function(a){a=="off"?($("html body").removeClass("fullscreen-mode"),$(".overlay").removeClass("show")):a=="on"?($("html body").addClass("fullscreen-mode"),$(".overlay").addClass("show")):($("html body").toggleClass("fullscreen-mode"),$(".overlay").toggleClass("show"))},u=function(){var a=$(".search input").val().toLowerCase().split(" ");$(".listnote").each(function(){var b=
$(this).data().content.toLowerCase();for(i=0,len=a.length;i<len;i++)if(b.indexOf(a[i])>-1)$(this).removeClass("search-hide");else{$(this).addClass("search-hide");break}});$(".selected:not(.search-hide,.tag-hide)").length==0&&($(".selected").removeClass("selected"),$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected"));g()},o=function(a,b){$(".status .progress .inner").length==0&&$(".status").html('<div class="progress"><div class="inner"></div></div>');$(".status .progress .inner").animate({width:a+
"%"},250,b)},N=$.throttle(500,!0,o),t=function(a){a=="clear"?($(".msg .inner").text(""),$(".msg").removeClass("show")):($(".msg .inner").text(a),$(".msg").addClass("show"))},O=function(a){return a.replace(/\n/g,"<br>").replace(/\r/g,"")},C=function(a){epoch_num=parseFloat(a);return time_dt=(new Date(epoch_num*1E3)).toISOString()},s=function(){var a=$("#"+$(".selected").data().key),b=a.prevAll(".on").eq(1),c=a.prevAll(".on").eq(0),f=a.nextAll(".on").eq(0),d=a.nextAll(".on").eq(1);a.hasClass("current")||
($(".current").removeClass("current"),a.addClass("current"));b.hasClass("prev2")||($(".prev2").removeClass("prev2"),b.addClass("prev2"));c.hasClass("prev1")||($(".prev1").removeClass("prev1"),c.addClass("prev1"));f.hasClass("next1")||($(".next1").removeClass("next1"),f.addClass("next1"));d.hasClass("next2")||($(".next2").removeClass("next2"),d.addClass("next2"))},g=function(){$(".selected").length!==1&&($(".selected").removeClass("selected"),$(".listnote:visible:first").addClass("selected"));var a=
$.parseJSON(localStorage.tagIndex).tags,b=[],c;for(c in a)b.push(a[c].name);var f=function(a){$("#note-template").clone().attr("id",a.key).addClass("note").insertAfter(d);var c=$("#"+a.key);c.data("syncnum",a.syncnum);c.children(".textarea").children(".ta-inner").children("textarea").val(a.content);var f=a.tags.join(" ");tagfield=c.children(".textarea").children(".tag-area").children("input");tagfield.val(f);tagfield.tagit({singleFieldDelimiter:" ",availableTags:b}).tagit({onTagAdded:function(){v($(".current").attr("id"))},
onTagRemoved:function(a,b){tagName=b.children(".tagit-label").text();v($(".current").attr("id"),tagName)}});c.addClass("on");$.inArray("markdown",a.systemtags)>-1&&p(a.key,"on");$.inArray("pinned",a.systemtags)>-1&&c.addClass("pinned");w("#"+a.key+" .textarea textarea")},d=$(".notes-placeholder");$(".listnote").each(function(){var a=$(this).data(),b=$("#"+a.key).length>0,c=b?$("#"+a.key):!1;$(this).css("display")=="none"?b&&c.removeClass("on").removeClass("current").removeClass("prev1").removeClass("prev2").removeClass("next1").removeClass("next2"):
b?(a.syncnum>c.data().syncnum?(c.remove(),f(a),c=$("#"+a.key)):c.addClass("on"),c.insertAfter(d)):(f(a),c=$("#"+a.key));d=c});s()},w=function(a){$(a).bind("keydown","esc",function(){$(".note-directions").removeClass("show");$(this).blur()});$(a).tabby();$(".maximize").click(function(){r()});$(".tagsinput div input").blur(function(){v($(".current").attr("id"))})},q=function(){$(".listnote").sortElements(function(a,b){if($(a).hasClass("pinned")){if(!$(b).hasClass("pinned"))return-1}else if($(b).hasClass("pinned")&&
!$(a).hasClass("pinned"))return 1;return parseFloat($(a).data("modifydate"))<parseFloat($(b).data("modifydate"))?1:-1});return"sorted"},k=function(){$(".selected").nextAll(".listnote:not(.hide):first");nextOne=$(".selected").nextAll(".listnote:visible:first");nextOne.length>0&&($(".selected").removeClass("selected"),nextOne.addClass("selected"),$(".prev2").removeClass("prev2"),$(".prev1").addClass("prev2").removeClass("prev1"),$(".current").addClass("prev1").removeClass("current"),$(".next1").addClass("current").removeClass("next1"),
$(".next2").addClass("next1").removeClass("next2"),$(".next1").nextAll(".on").eq(0).addClass("next2"),D())},l=function(){prevOne=$(".selected").prevAll(".listnote:visible:first");prevOne.length>0&&($(".selected").removeClass("selected"),prevOne.addClass("selected"),$(".next2").removeClass("next2"),$(".next1").removeClass("next1").addClass("next2"),$(".current").removeClass("current").addClass("next1"),$(".prev1").removeClass("prev1").addClass("current"),$(".prev2").removeClass("prev2").addClass("prev1"),
$(".prev1").prevAll(".on").eq(0).addClass("prev2"),D())},p=function(a,b,c){a=a?a:$(".current").attr("id");b==void 0&&(b=$("#"+a).hasClass("markdown-on")?"off":"on");c!="temp"&&(c="stick");var f=$("#"+a),a=$("#list-"+a),d=a.data(),g=f.children(".textarea");if(b=="on")f.removeClass("markdown-off").addClass("markdown-on"),c=(new Markdown.Converter).makeHtml("# "+g.children(".ta-inner").children("textarea").val()),g.children(".ta-inner").replaceWith('<div class="markdown">'+c+"</div>"),g.children(".markdown").children("h1:first").addClass("first-h1"),
g.children(".markdown").children("p").each(function(){$(this).html(O($(this).html()))}),$.inArray("markdown",d.systemtags)<0&&(d.systemtags.push("markdown"),d.syncnum++,localStorage.setItem(d.key,JSON.stringify(a.data())),m(d.key),n());else if(b=="off")f.removeClass("markdown-on").addClass("markdown-off"),b=d.content,g.children(".markdown").replaceWith('<div class="ta-inner"><textarea>'+b+"</textarea></div>"),w(g.children(".ta-inner").children("textarea")),c=="stick"&&(c=a.data().systemtags.indexOf("markdown"),
c>-1&&d.systemtags.splice(c,1),d.syncnum+=1,localStorage.setItem(d.key,JSON.stringify(a.data())),m(d.key),n())},E=function(a){t("Working...");a==void 0&&$(".selected").length>-1&&(a=$(".selected"));a.data().systemtags.indexOf("pinned")>-1?a.data().systemtags.splice(a.data().systemtags.indexOf("pinned"),1):a.data().systemtags.push("pinned");a.data().syncnum=a.data().syncnum+1;localStorage.setItem(a.data().key,JSON.stringify(a.data()));m(a.data().key);q();g();t("clear")},F=function(){createdate=(new Date).getTime();
newid="notestack"+createdate;newdata={key:newid,createdate:createdate/1E3,modifydate:createdate/1E3,tags:[],systemtags:[],content:"new note"};localStorage.setItem(newid,JSON.stringify(newdata));index=$.parseJSON(localStorage.index);index.data.push(newdata);localStorage.index=JSON.stringify(index);m(newid);q();$(".selected").removeClass("selected");$("#list-"+newid).addClass("selected");g();$(".current .textarea textarea").select()},D=function(){$(".selected").position().top<0&&$(".list").scrollTop($(".list").scrollTop()+
$(".selected").position().top);$(".selected").position().top+$(".listnote").height()>$(".list").height()&&$(".list").scrollTop($(".list").scrollTop()+$(".selected").position().top-$(".list").height()+$(".listnote").height())},G=function(){$("html body").removeClass("tag-filter");$(".active-tag-filter").remove();$(".listnote").removeClass("tag-hide");$(".selected").removeClass("selected");$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");$(".search input").val("");u()},P=function(){$(".version-item").sortElements(function(a,
b){return parseFloat($(a).data("version"))<parseFloat($(b).data("version"))?1:-1});return"sorted"},Q=function(a,b){return $.Deferred(function(c){$.ajax("/sn.php",{type:"POST",data:{action:"noteversion",email:email,token:token,notekey:a,noteversion:b},success:function(b){note=$.parseJSON(b);$("#version-template").clone().attr("id",a+"version"+note.version).addClass("version-item").appendTo(".versions .versions-left").data(note);thisversion=$("#"+a+"version"+note.version);thisversion.children(".version-date").html('<abbr class="timeago" title="'+
C(note.versiondate)+'"></abbr>');$(".timeago").timeago();P();c.resolve()},error:function(){return"false"}})}).promise()},R=function(a){$(".version-item").remove();var b=[];noteobj=$.parseJSON(localStorage.getItem(a));curversion=noteobj.version;for(i=minversion=noteobj.minversion;i<=curversion;i++)b.push(Q(a,i));$.when.apply(null,b).done(function(){$(".version-item").each(function(){if($(this).data().version>minversion){prev=$(this).data().version-1;var b=$("#"+a+"version"+prev).data().content,f=$(this).data().content,
d=new diff_match_patch,b=d.diff_main(b,f);d.diff_cleanupSemantic(b);ds=d.diff_prettyHtml(b);$(this).data("diff",ds)}else $(this).data().version==minversion&&$(this).data("diff",$(this).data().content)});$(".versions-right .right-inner").html($(".version-item:first").data().diff);$(".version-item:first").addClass("version-select")})},H=function(a){$(".versions").hasClass("show")?h():(r("off"),$(".versions").addClass("show"),$(".overlay").addClass("show"),$(".versions-right .right-inner").html(""),
R(a))},S=function(){return $.Deferred(function(a){postData={action:"tagIndex",email:localStorage.email,token:localStorage.token};$.ajax("/sn.php",{type:"POST",data:postData,timeout:3E3,success:function(b){if(b=="401")window.location="/?login=expired";localStorage.tagIndex=b;a.resolve()},error:function(){$(".status").text("Working offline. Changes will sync when connected.");$(".status-div").removeClass("loading")}})})},x=function(){$(".labels").hasClass("show")?h():(r("off"),T(),$(".labels").addClass("show"),
$(".overlay").addClass("show"),$(".labels input").val("").select());return!1},T=function(){$(".labels .item").remove();var a=$.parseJSON(localStorage.tagIndex);$(".listnote");for(i=0;i<a.count;i++)tagname=a.tags[i].name,tagid=tagname.replace("@","").replace(".",""),j=0,$(".listnote").each(function(){$.inArray(tagname,$(this).data().tags)>-1&&($(this).data(),j++)}),$("#label-template").clone().attr("id",tagid).addClass("item").appendTo(".labels .items"),$("#"+tagid).children(".name").html("<pre>"+
tagname+"</pre>"),$("#"+tagid).children(".notecount").text(j+" notes");$(".labels .item").sortElements(function(a,c){return parseFloat($(a).children(".notecount").text())<parseFloat($(c).children(".notecount").text())?1:-1})},I=function(){query=$(".labels input").val().toLowerCase();$(".labels .item").each(function(){$(this).children(".name").text().toLowerCase().indexOf(query)>-1?$(this).removeClass("hide"):$(this).addClass("hide")});$(".label-select").removeClass("label-select");$(".labels .item:not(.hide):first").addClass("label-select")},
v=function(a,b){var c=$("#"+a+" .tag-area input").val().split(" "),f=c.indexOf(b);f!=-1&&c.splice(f,1);note=$.parseJSON(localStorage[a]);if(note.tags!=c)note.tags=c,note.syncnum+=1,localStorage[a]=JSON.stringify(note),m(a),n()},y=function(a){t("Working...");$(".labels input").val("").blur();h();J("partial");$("html body").addClass("tag-filter");$("#tag-filter-template").clone().attr("id","").addClass("active-tag-filter").insertBefore(".list");$(".active-tag-filter .inner .tag-filter-label").text(a);
$(".listnote").each(function(){$.inArray(a,$(this).data().tags)>-1||$(this).addClass("tag-hide")});$(".selected").removeClass("selected");$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");g();t("clear")},J=function(a){$(".tag-hide").removeClass("tag-hide");$(".active-tag-filter").remove();a!=="partial"&&($("html body").removeClass("tag-filter"),$(".selected").removeClass("selected"),$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected"),g())},K=function(a){return $.Deferred(function(b){$.ajax("/sn.php",
{type:"POST",data:{action:"note",email:email,token:token,notekey:a},success:function(a){note=$.parseJSON(a);localStorage.setItem(note.key,a);m(note.key);b.resolve()},error:function(){}})}).promise()},L=function(a){var b=$.Deferred(),c={action:"sendnote",email:email,token:token,notekey:a.key,notebody:JSON.stringify(a)};if(a.key.substr(0,9)=="notestack")delete c.notekey,delete a.key,c.notebody=JSON.stringify(a);$.ajax("/sn.php",{type:"POST",data:c,success:function(c){note=$.parseJSON(c);if(!("content"in
note))note.content=a.content;index=$.parseJSON(localStorage.index);for(i=0;i<=index.data.length-1;i++)if(index.data[i].key.substr(0,9)=="notestack"){$("#list-"+index.data[i].key).remove();localStorage.removeItem(index.data[i].key);index.data.splice(i,1);break}localStorage.index=JSON.stringify(index);localStorage.setItem(note.key,JSON.stringify(note));m(note.key);$("#list-"+note.key).addClass("selected");q();g();b.resolve()},error:function(){}});return b.promise()},U=function(){return $.Deferred(function(a){function b(h){for(c=
f;c<=f+h;c++){N(50+Math.round(50*(c/index.data.length)));if(!index.data[c]){a.resolve();var n=!0;break}localStorage.getItem(index.data[c].key)?(localNote=$.parseJSON(localStorage.getItem(index.data[c].key)),localNote.key.substr(0,9)=="notestack"?d.push(L(localNote)):localNote.syncnum<index.data[c].syncnum?d.push(K(localNote.key)):localNote.syncnum>index.data[c].syncnum?d.push(L(localNote)):m(localNote.key)):d.push(K(index.data[c].key));index.data[c].deleted==0?k++:index.data[c].deleted==1&&l++}if(n==
!0)return!1;d.length>0?$.when.apply(null,d).done(function(){f=f+h+1;d=[];q();g();b(h)}):(f=f+h+1,d=[],b(h))}index=$.parseJSON(localStorage.index);var c=0,f=0,d=[],k=0,l=0;b(20);$(".note-data").text(k+" notes | ");$(".data_notes").text(k);$(".data_deleted").text(l)}).promise()},M=function(a){return $.Deferred(function(b){$.ajax("/sn.php",{type:"POST",data:a,timeout:5E3,success:function(c){if(c=="401")window.location="/?login=expired";newIndex=$.parseJSON(c);if(localStorage.index){existingIndex=$.parseJSON(localStorage.index);
for(i=0;i<=newIndex.data.length-1;i++){for(j=0;j<=existingIndex.data.length-1;j++)if(existingIndex.data[j].key==newIndex.data[i].key){existingIndex.data.splice(j,1);break}existingIndex.data.push(newIndex.data[i])}existingIndex.count=existingIndex.data.length}else existingIndex=newIndex;localStorage.setItem("mark",newIndex.mark?newIndex.mark:"DONE");localStorage.setItem("index",JSON.stringify(existingIndex));localStorage.setItem("indexDate",newIndex.time);localStorage.mark=="DONE"?(localStorage.removeItem("mark"),
b.resolve()):(a.mark=localStorage.mark,$.when(M(a)).done(function(){b.resolve()}))},error:function(){$(".status").text("Working offline. Changes will sync when connected.");$(".status-div").removeClass("loading")}})}).promise()},n=function(a){o(5);localStorage.token?$.when(S()).done(function(){o(25);a=="full"?mark=since="":localStorage.mark?(since="",mark=localStorage.mark):(since=localStorage.indexDate?localStorage.indexDate:"",mark="");email=localStorage.email;token=localStorage.token;$.when(M({action:"index",
email:email,token:token,since:since,mark:mark,length:100})).done(function(){o(50);(new Date).getTime();$.when(U()).done(function(){o(100,function(){indexDate=C(localStorage.indexDate);$(".status").html('synced <abbr class="timeago" title="'+indexDate+'"></abbr>');$(".timeago").timeago();$(".status-div").removeClass("loading");q();g()})})})}):window.location="/"},m=function(a){return $.Deferred(function(b){note=$.parseJSON(localStorage.getItem(a));note.deleted!=1&&($("#list-"+note.key).length>0?$("#list-"+
note.key).removeData().data(note):$("#listnote-template").clone().attr("id","list-"+note.key).addClass("listnote").appendTo(".list").data(note),thisnote=$("#list-"+note.key),thisnote.removeClass("pinned"),thisnote.data().systemtags.indexOf("pinned")>-1&&thisnote.addClass("pinned"),titlebreak=thisnote.data().content.indexOf("\n"),titlebreak<0?(thisnote.children(".title").text(thisnote.data().content.substring(0,100)),thisnote.children(".preview").text("")):(thisnote.children(".title").text(thisnote.data().content.substring(0,
titlebreak)),thisnote.children(".preview").text(thisnote.data().content.substr(titlebreak,100))),b.resolve())}).promise()},z=function(a){var b=$("html body").attr("class").split(" ");for(i=0,len=b.length;i<len;i++)b[i].substr(0,6)=="theme-"&&(b[i]="");$(".appearance .item.theme").siblings(".theme").addClass("dim");$("#"+a).removeClass("dim");$("html body").attr("class",b.join(" "));$("html body").addClass(a);localStorage.theme=a},A=function(a){var b=$("html body").attr("class").split(" ");for(i=0,
len=b.length;i<len;i++)b[i].substr(0,5)=="font-"&&(b[i]="");$(".appearance .font").siblings(".font").addClass("dim");$("#"+a).removeClass("dim");$("html body").attr("class",b.join(" "));$("html body").addClass(a);localStorage.font=a},B=function(a){var b=$("html body").attr("class").split(" ");for(i=0,len=b.length;i<len;i++)b[i].substr(0,9)=="fontsize-"&&(b[i]="");$(".appearance .fontsize").siblings(".fontsize").addClass("dim");$("#"+a).removeClass("dim");$("html body").attr("class",b.join(" "));$("html body").addClass(a);
localStorage.fontsize=a},V=function(){localStorage.theme?(themename=localStorage.theme,z(themename)):z("theme-moleskine");$(".appearance .item.theme").click(function(){themename=$(this).attr("id");$(this).removeClass("dim");z(themename)});localStorage.font?(fontname=localStorage.font,A(fontname)):A("font-mono");$(".appearance .font").click(function(){var a=$(this).attr("id");A(a)});localStorage.fontsize?(fontsize=localStorage.fontsize,B(fontsize)):B("fontsize-12");$(".appearance .fontsize").click(function(){var a=
$(this).attr("id");B(a)});localStorage.tagPref=="no-tags"&&($("html body").addClass("no-tags"),$(".tag-pref").addClass("dim"));$(".appearance .tag-pref").click(function(){$(this).hasClass("dim")?($(this).removeClass("dim"),$("html body").removeClass("no-tags"),localStorage.removeItem("tagPref")):($(this).addClass("dim"),$("html body").addClass("no-tags"),localStorage.setItem("tagPref","no-tags"))});$(".note .textarea textarea").live("focus",function(){$(".current .note-directions").addClass("show");
$("div.current").addClass("highlight")});$(".note .textarea textarea").live("blur",function(){var a=$(this).parent().parent().parent().attr("id"),b=$("#list-"+a).data();$.inArray("markdown",b.systemtags)>-1&&p(a,"on");if($(this).val()!=b.content)blurnote=$.parseJSON(localStorage.getItem(a)),blurnote.content=$(this).val(),blurnote.modifydate=(new Date).getTime()/1E3,blurnote.syncnum+=1,localStorage.setItem(a,JSON.stringify(blurnote)),m(blurnote.key),n();$(".current .note-directions").removeClass("show");
$(".current").removeClass("highlight")});$(".addnote").click(function(){F()});$(".tags").click(function(){x()});$(".reload").click(function(){n()});$(".close").click(function(){h()});$(".delete").live("click",function(){$(this).parent().children(".message").toggleClass("show").html('Mark this note as deleted?  <a href="#" class="confirmDelete">Delete</a> / <a href="#" class="cancelDelete">Cancel</a>')});$(".confirmDelete").live("click",function(){var a=notekey=$(this).parent().parent().attr("id");
note=$("#list-"+a).data();note.deleted=1;note.modifydate=(new Date).getTime()/1E3;note.syncnum+=1;localStorage.setItem(note.key,JSON.stringify(note));$("#list-"+a).remove();$("#"+a).remove();$(".listnote:first").addClass(".selected");g();return!1});$(".cancelDelete").live("click",function(){$(".message").removeClass("show");return!1});$(".listnote").live("click",function(){var a=$(".selected"),b=$(".listnote:visible").index(a),c=$(".listnote:visible").index(this),a=Math.abs(c-b),b=b<c?"next":"prev";
if(a<=6)if(b=="next")for(c=1;c<=a;c++)k();else{if(b=="prev")for(c=1;c<=a;c++)l()}else b=="next"?($(".selected").removeClass("selected"),$(".listnote:visible").eq(c-4).addClass("selected"),s(),k(),k(),k(),k()):b=="prev"&&($(".selected").removeClass("selected"),$(".listnote:visible").eq(c+4).addClass("selected"),s(),l(),l(),l(),l())});$(".note").live("click",function(){$(".current").removeClass("current");$(this).addClass("current");$(".selected").removeClass("selected");$("#list-"+$(this).attr("id")).addClass("selected");
s()});$(".textarea:not(a)").live("click",function(a){$(a.target).parents(".tag-area").length>0||(p($(this).parent().attr("id"),"off","temp"),$(this).children(".ta-inner").children("textarea").focus())});$(".markdown a").live("click",function(){$(this).attr("target","_blank")});$(".settings_icon").click(function(){$(".settings").toggleClass("show")});$(".userdata").click(function(){$(".settings").toggleClass("show");$(".settings_pane").removeClass("show");$(".settings_pane.data").addClass("show")});
$(".settings .tabs a").click(function(a){$(".settings_pane").removeClass("show");$(".settings_pane."+$(this).text().replace(" ","_")).addClass("show");a.preventDefault();return!1});$(".versions-button").live("mousedown",function(){notekey=$(this).parent().attr("id");H(notekey)});$(".version-item").live("mousedown",function(){$(".diff-view").hasClass("depressed")?$(".versions-right .right-inner").html($(this).data().diff):$(".versions-right .right-inner").html('<textarea disabled="disabled">'+$(this).data().content+
"</textarea>");$(".version-select").removeClass("version-select");$(this).addClass("version-select")});$(".versions-toolbar .diff-view,.versions-toolbar .text-view").live("mousedown",function(){$(".versions-toolbar .depressed").removeClass("depressed");$(this).addClass("depressed");$(".version-select").length<1&&$(".version-item:first").addClass(".version-select");$(this).hasClass("diff-view")?$(".versions-right .right-inner").html($(".version-select").data().diff):$(this).hasClass("text-view")&&
$(".versions-right .right-inner").html('<textarea disabled="disabled">'+$(".version-select").data().content+"</textarea>")});$(".overlay").click(function(){h()});$(".versions-toolbar .revert-version").live("mousedown",function(){if($(".version-select").length==1)return notekey=$(".version-select").attr("id").split("version").shift(),verContent=$(".version-select").data().content,$(".selected").removeClass("selected"),$("#list-"+notekey).addClass("selected"),$(".current").removeClass("current"),$("#"+
notekey).addClass("current"),s(),h(),$(".note.current .textarea textarea").val(verContent).focus(),!1});$(".clear-tag-filter").live("mouseup",function(){J()});$(".labels .item").live("mouseup",function(){y($(this).attr("id"))});$(".tagit-label").live("mouseup",function(){y($(this).text())});$(".markdown-button").live("mouseup",function(){p($(this).parent().attr("id"))});$(".pin-button").live("mouseup",function(){E()});$(".cleardata").click(function(){localStorage.clear()});$(document).bind("keydown",
"esc",function(){h()});$(document).bind("keydown","j",function(){k()});$(document).bind("keydown","right",function(){k()});$(document).bind("keydown","down",function(){k()});$(document).bind("keydown","k",function(){l()});$(document).bind("keydown","left",function(){l()});$(document).bind("keydown","up",function(){l()});$(document).bind("keydown","return",function(){$(".current").hasClass("markdown-on")&&p($(".current").attr("id"),"off","temp");$(".note.current .textarea textarea").focus();return!1});
$(document).bind("keydown","tab",function(){$(".note.current .textarea textarea").focus();return!1});$(document).bind("keydown","shift+/",function(){$(".settings").toggleClass("show")});$(document).bind("keydown","/",function(){$(".search input").select();return!1});$(document).bind("keydown","c",function(){F();return!1});$(document).bind("keydown","s",function(){n();return!1});$(document).bind("keydown","p",function(){E($(".selected"))});$(document).bind("keydown","f",function(){r()});$(document).bind("keydown",
...
function(){$(this).val("");u();$(this).blur();return!1});$(".search input").bind("keydown","down",function(){$(this).blur();k();return!1});$(".search input").bind("keydown","up",function(){$(this).blur();l();return!1});$(".labels input").keyup(function(a){a=a.which;a!=38&&a!=40&&a!=13&&I()});$(".labels input").bind("keydown","esc",function(){$(this).val("");$(this).blur();h();return!1});$(".labels input").bind("keydown","down",function(){nextOne=$(".label-select").nextAll(".item:not(.hide):first");
...
I()});return!1});w(".note .textarea textarea")};$(function(){V();if(localStorage.email){$(".username").text(localStorage.email);if(localStorage.index){o(10);if(localStorage.index){index=$.parseJSON(localStorage.index);for(i=0;i<=index.data.length-1;i++)localStorage.getItem(index.data[i].key)!=null&&m(index.data[i].key)}$(".listnote").length>0&&(q(),g())}(new Date).getTime();n()}else window.location="/"})}();