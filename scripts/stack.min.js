...
// NOTESTACK CODE
// http://notestack.me
var notestack=function(){$=jQuery.noConflict();var k=function(){$(".show:not(.settings_pane)").removeClass("show");r("off")},r=function(a){a=="off"?($("html body").removeClass("fullscreen-mode"),$(".overlay").removeClass("show")):a=="on"?($("html body").addClass("fullscreen-mode"),$(".overlay").addClass("show")):($("html body").toggleClass("fullscreen-mode"),$(".overlay").toggleClass("show"))},u=function(){query=$(".search input").val().toLowerCase();$(".listnote").each(function(){$(this).data().content.toLowerCase().indexOf(query)>
-1?$(this).removeClass("search-hide"):$(this).addClass("search-hide")});$(".selected:not(.search-hide,.tag-hide)").length==0&&($(".selected").removeClass("selected"),$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected"));l()},n=function(a,b){$(".status .progress .inner").length==0&&$(".status").html('<div class="progress"><div class="inner"></div></div>');$(".status .progress .inner").animate({width:a+"%"},250,b)},L=$.throttle(500,!0,n),M=function(a){return a.replace(/\n/g,"<br>").replace(/\r/g,
"")},C=function(a){epoch_num=parseFloat(a);return time_dt=(new Date(epoch_num*1E3)).toISOString()},o=function(){$(".current").length!=1&&($(".selected").length!=1?($(".selected").removeClass("selected"),$(".listnote:first").addClass("selected")):$("#"+$(".selected").data().key).addClass("current"));$(".prev2").removeClass("prev2");$(".prev1").removeClass("prev1");$(".next1").removeClass("next1");$(".next2").removeClass("next2");$(".current").prev(".note").prev(".note").addClass("prev2");$(".current").prev(".note").addClass("prev1");
$(".current").next(".note").addClass("next1");$(".current").next(".note").next(".note").addClass("next2")},l=function(){console.log("start refresh cards");$(".selected").length!=1&&($(".selected").removeClass("selected"),$(".listnote:first").addClass("selected"));$(".note").remove();var a=$.parseJSON(localStorage.tagIndex).tags,b=[],c;for(c in a)b.push(a[c].name);$(".listnote").not(".search-hide,.tag-hide").each(function(){var a=$(this).data();$("#note-template").clone().attr("id",a.key).addClass("note").appendTo(".window");
var c=$("#"+a.key);c.children(".textarea").children("textarea").val(a.content);var h=a.tags.join(" ");tagfield=c.children(".textarea").children(".tag-area").children("input");tagfield.val(h);tagfield.tagit({singleFieldDelimiter:" ",availableTags:b}).tagit({onTagAdded:function(){v($(".current").attr("id"))},onTagRemoved:function(a,b){tagName=b.children(".tagit-label").text();v($(".current").attr("id"),tagName)}});$.inArray("markdown",a.systemtags)>-1&&p(a.key,"on");$.inArray("pinned",a.systemtags)>
-1&&c.addClass("pinned")});$("#"+$(".selected").data().key).addClass("current");w(".note .textarea textarea");o();console.log("finish refresh cards")},w=function(a){$(a).bind("keydown","esc",function(){$(".note-directions").removeClass("show");$(this).blur()});$(a).tabby();$(".maximize").click(function(){r()});$(".tagsinput div input").blur(function(){v($(".current").attr("id"))})},q=function(){$(".listnote").sortElements(function(a,b){if($(a).hasClass("pinned")){if(!$(b).hasClass("pinned"))return-1}else if($(b).hasClass("pinned")&&
!$(a).hasClass("pinned"))return 1;return parseFloat($(a).data("modifydate"))<parseFloat($(b).data("modifydate"))?1:-1});return"sorted"},s=function(){nextOne=$(".selected").nextAll(".listnote:not(.search-hide,.tag-hide):first");nextOne.length>0&&($(".selected").removeClass("selected"),nextOne.addClass("selected"),$(".current").removeClass("current").next(".note").addClass("current"),o(),D())},t=function(){prevOne=$(".selected").prevAll(".listnote:not(.search-hide,.tag-hide):first");prevOne.length>
0&&($(".selected").removeClass("selected"),prevOne.addClass("selected"),$(".current").removeClass("current").prev(".note").addClass("current"),o(),D())},p=function(a,b,c){a=a?a:$(".current").attr("id");b==void 0&&(b=$("#"+a).hasClass("markdown-on")?"off":"on");c!="temp"&&(c="stick");var f=$("#"+a),a=$("#list-"+a),d=a.data(),h=f.children(".textarea");if(b=="on")f.removeClass("markdown-off").addClass("markdown-on"),c=(new Markdown.Converter).makeHtml("# "+h.children("textarea").val()),h.children("textarea").replaceWith('<div class="markdown">'+
c+"</div>"),h.children(".markdown").children("h1:first").addClass("first-h1"),h.children(".markdown").children("p").each(function(){$(this).html(M($(this).html()))}),$.inArray("markdown",d.systemtags)<0&&(d.systemtags.push("markdown"),d.syncnum++,localStorage.setItem(d.key,JSON.stringify(a.data())),m(d.key),g());else if(b=="off")f.removeClass("markdown-on").addClass("markdown-off"),b=d.content,h.children(".markdown").replaceWith("<textarea>"+b+"</textarea>"),w(h.children("textarea")),c=="stick"&&
(c=indexOf("markdown",a.data().systemtags),c>-1&&d.systemtags.splice(c,1),d.syncnum++,localStorage.setItem(d.key,JSON.stringify(a.data())),m(d.key),g())},E=function(a){a==void 0&&$(".selected").length>-1&&(a=$(".selected"));a.data().systemtags.indexOf("pinned")>-1?a.data().systemtags.splice(a.data().systemtags.indexOf("pinned"),1):a.data().systemtags.push("pinned");a.data().syncnum=a.data().syncnum+1;localStorage.setItem(a.data().key,JSON.stringify(a.data()));m(a.data().key);q();l()},F=function(){createdate=
(new Date).getTime();newid="notestack"+createdate;newdata={key:newid,createdate:createdate/1E3,modifydate:createdate/1E3,tags:[],systemtags:[],content:"new note"};localStorage.setItem(newid,JSON.stringify(newdata));index=$.parseJSON(localStorage.index);index.data.push(newdata);localStorage.index=JSON.stringify(index);m(newid);q();$(".selected").removeClass("selected");$("#list-"+newid).addClass("selected");l();$(".current .textarea textarea").select()},D=function(){$(".selected").position().top<0&&
$(".list").scrollTop($(".list").scrollTop()+$(".selected").position().top);$(".selected").position().top+$(".listnote").height()>$(".list").height()&&$(".list").scrollTop($(".list").scrollTop()+$(".selected").position().top-$(".list").height()+$(".listnote").height())},N=function(){$(".version-item").sortElements(function(a,b){return parseFloat($(a).data("version"))<parseFloat($(b).data("version"))?1:-1});return"sorted"},O=function(a,b){return $.Deferred(function(c){$.ajax("/sn.php",{type:"POST",
data:{action:"noteversion",email:email,token:token,notekey:a,noteversion:b},success:function(b){note=$.parseJSON(b);$("#version-template").clone().attr("id",a+"version"+note.version).addClass("version-item").appendTo(".versions .versions-left").data(note);thisversion=$("#"+a+"version"+note.version);thisversion.children(".version-date").html('<abbr class="timeago" title="'+C(note.versiondate)+'"></abbr>');$(".timeago").timeago();N();c.resolve()},error:function(){return"false"}})}).promise()},P=function(a){$(".version-item").remove();
var b=[];noteobj=$.parseJSON(localStorage.getItem(a));curversion=noteobj.version;for(i=minversion=noteobj.minversion;i<=curversion;i++)b.push(O(a,i));$.when.apply(null,b).done(function(){$(".version-item").each(function(){if($(this).data().version>minversion){prev=$(this).data().version-1;var b=$("#"+a+"version"+prev).data().content,f=$(this).data().content,d=new diff_match_patch,b=d.diff_main(b,f);d.diff_cleanupSemantic(b);ds=d.diff_prettyHtml(b);$(this).data("diff",ds)}else $(this).data().version==
minversion&&$(this).data("diff",$(this).data().content)});$(".versions-right .right-inner").html($(".version-item:first").data().diff);$(".version-item:first").addClass("version-select")})},G=function(a){$(".versions").hasClass("show")?k():(r("off"),$(".versions").addClass("show"),$(".overlay").addClass("show"),$(".versions-right .right-inner").html(""),P(a))},Q=function(){return $.Deferred(function(a){postData={action:"tagIndex",email:localStorage.email,token:localStorage.token};$.ajax("/sn.php",
{type:"POST",data:postData,timeout:3E3,success:function(b){if(b=="401")window.location="/?login=expired";localStorage.tagIndex=b;a.resolve()},error:function(){$(".status").text("Working offline. Changes will sync when connected.");$(".status-div").removeClass("loading")}})})},x=function(){$(".labels").hasClass("show")?k():(r("off"),R(),$(".labels").addClass("show"),$(".overlay").addClass("show"),$(".labels input").val("").select());return!1},R=function(){$(".labels .item").remove();var a=$.parseJSON(localStorage.tagIndex);
$(".listnote");for(i=0;i<a.count;i++)tagname=a.tags[i].name,tagid=tagname.replace("@","").replace(".",""),j=0,$(".listnote").each(function(){$.inArray(tagname,$(this).data().tags)>-1&&j++}),$("#label-template").clone().attr("id",tagid).addClass("item").appendTo(".labels .items"),$("#"+tagid).children(".name").html("<pre>"+tagname+"</pre>"),$("#"+tagid).children(".notecount").text(j+" notes");$(".labels .item").sortElements(function(a,c){return parseFloat($(a).children(".notecount").text())<parseFloat($(c).children(".notecount").text())?
1:-1})},S=function(){query=$(".labels input").val().toLowerCase();$(".labels .item").each(function(){$(this).children(".name").text().toLowerCase().indexOf(query)>-1?$(this).removeClass("hide"):$(this).addClass("hide")});$(".label-select").removeClass("label-select");$(".labels .item:not(.hide):first").addClass("label-select")},v=function(a,b){var c=$("#"+a+" .tag-area input").val().split(" "),f=c.indexOf(b);f!=-1&&c.splice(f,1);note=$.parseJSON(localStorage[a]);if(note.tags!=c)note.tags=c,note.syncnum+=
1,localStorage[a]=JSON.stringify(note),m(a),g()},y=function(a){$(".labels input").val("").blur();H();k();$("html body").addClass("tag-filter");$("#tag-filter-template").clone().attr("id","").addClass("active-tag-filter").insertBefore(".list");$(".active-tag-filter .inner .tag-filter-label").text(a);$(".listnote").each(function(){$.inArray(a,$(this).data().tags)>-1||$(this).addClass("tag-hide")});$(".selected").removeClass("selected");$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");
l()},H=function(){console.log("start unfilter label");$("html body").removeClass("tag-filter");$(".active-tag-filter").remove();$(".tag-hide").removeClass("tag-hide");$(".selected").removeClass("selected");$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");console.log("midpoint");l();console.log("finish unfilter labels")},I=function(a){return $.Deferred(function(b){$.ajax("/sn.php",{type:"POST",data:{action:"note",email:email,token:token,notekey:a},success:function(a){note=$.parseJSON(a);
localStorage.setItem(note.key,a);m(note.key);b.resolve()},error:function(){}})}).promise()},J=function(a){var b=$.Deferred(),c={action:"sendnote",email:email,token:token,notekey:a.key,notebody:JSON.stringify(a)};if(a.key.substr(0,9)=="notestack")delete c.notekey,delete a.key,c.notebody=JSON.stringify(a);$.ajax("/sn.php",{type:"POST",data:c,success:function(c){note=$.parseJSON(c);if(!("content"in note))note.content=a.content;index=$.parseJSON(localStorage.index);for(i=0;i<=index.data.length-1;i++)if(index.data[i].key.substr(0,
9)=="notestack"){$("#list-"+index.data[i].key).remove();localStorage.removeItem(index.data[i].key);index.data.splice(i,1);break}localStorage.index=JSON.stringify(index);localStorage.setItem(note.key,JSON.stringify(note));m(note.key);$("#list-"+note.key).addClass("selected");q();l();b.resolve()},error:function(){}});return b.promise()},T=function(){return $.Deferred(function(a){function b(g){for(c=f;c<=f+g;c++){L(50+Math.round(50*(c/index.data.length)));if(!index.data[c]){a.resolve();var n=!0;break}localStorage.getItem(index.data[c].key)?
(localNote=$.parseJSON(localStorage.getItem(index.data[c].key)),localNote.key.substr(0,9)=="notestack"?d.push(J(localNote)):localNote.syncnum<index.data[c].syncnum?d.push(I(localNote.key)):localNote.syncnum>index.data[c].syncnum?d.push(J(localNote)):m(localNote.key)):d.push(I(index.data[c].key));index.data[c].deleted==0?h++:index.data[c].deleted==1&&k++}if(n==!0)return!1;d.length>0?$.when.apply(null,d).done(function(){f=f+g+1;d=[];q();l();b(g)}):(f=f+g+1,d=[],b(g))}index=$.parseJSON(localStorage.index);
var c=0,f=0,d=[],h=0,k=0;b(20);$(".note-data").text(h+" notes | ");$(".data_notes").text(h);$(".data_deleted").text(k)}).promise()},K=function(a){return $.Deferred(function(b){$.ajax("/sn.php",{type:"POST",data:a,timeout:5E3,success:function(c){if(c=="401")window.location="/?login=expired";newIndex=$.parseJSON(c);if(localStorage.index){existingIndex=$.parseJSON(localStorage.index);for(i=0;i<=newIndex.data.length-1;i++){for(j=0;j<=existingIndex.data.length-1;j++)if(existingIndex.data[j].key==newIndex.data[i].key){existingIndex.data.splice(j,
1);break}existingIndex.data.push(newIndex.data[i])}existingIndex.count=existingIndex.data.length}else existingIndex=newIndex;localStorage.setItem("mark",newIndex.mark?newIndex.mark:"DONE");localStorage.setItem("index",JSON.stringify(existingIndex));localStorage.setItem("indexDate",newIndex.time);localStorage.mark=="DONE"?(localStorage.removeItem("mark"),b.resolve()):(a.mark=localStorage.mark,$.when(K(a)).done(function(){b.resolve()}))},error:function(){$(".status").text("Working offline. Changes will sync when connected.");
$(".status-div").removeClass("loading")}})}).promise()},g=function(a){n(5);localStorage.token?$.when(Q()).done(function(){n(25);a=="full"?mark=since="":localStorage.mark?(since="",mark=localStorage.mark):(since=localStorage.indexDate?localStorage.indexDate:"",mark="");email=localStorage.email;token=localStorage.token;$.when(K({action:"index",email:email,token:token,since:since,mark:mark,length:100})).done(function(){n(50);(new Date).getTime();$.when(T()).done(function(){n(100,function(){indexDate=
C(localStorage.indexDate);$(".status").html('synced <abbr class="timeago" title="'+indexDate+'"></abbr>');$(".timeago").timeago();$(".status-div").removeClass("loading");q();l()})})})}):window.location="/"},m=function(a){return $.Deferred(function(b){note=$.parseJSON(localStorage.getItem(a));note.deleted!=1&&($("#list-"+note.key).length>0?$("#list-"+note.key).removeData().data(note):$("#listnote-template").clone().attr("id","list-"+note.key).addClass("listnote").appendTo(".list").data(note),thisnote=
$("#list-"+note.key),thisnote.removeClass("pinned"),thisnote.data().systemtags.indexOf("pinned")>-1&&thisnote.addClass("pinned"),titlebreak=thisnote.data().content.indexOf("\n"),titlebreak<0?(thisnote.children(".title").text(thisnote.data().content.substring(0,100)),thisnote.children(".preview").text("")):(thisnote.children(".title").text(thisnote.data().content.substring(0,titlebreak)),thisnote.children(".preview").text(thisnote.data().content.substr(titlebreak,100))),b.resolve())}).promise()},z=
function(a){var b=$("html body").attr("class").split(" ");for(i=0,len=b.length;i<len;i++)b[i].substr(0,6)=="theme-"&&(b[i]="");$(".appearance .item.theme").siblings(".theme").addClass("dim");$("#"+a).removeClass("dim");$("html body").attr("class",b.join(" "));$("html body").addClass(a);localStorage.theme=a},A=function(a){var b=$("html body").attr("class").split(" ");for(i=0,len=b.length;i<len;i++)b[i].substr(0,5)=="font-"&&(b[i]="");$(".appearance .font").siblings(".font").addClass("dim");$("#"+a).removeClass("dim");
$("html body").attr("class",b.join(" "));$("html body").addClass(a);localStorage.font=a},B=function(a){var b=$("html body").attr("class").split(" ");for(i=0,len=b.length;i<len;i++)b[i].substr(0,9)=="fontsize-"&&(b[i]="");$(".appearance .fontsize").siblings(".fontsize").addClass("dim");$("#"+a).removeClass("dim");$("html body").attr("class",b.join(" "));$("html body").addClass(a);localStorage.fontsize=a},U=function(){localStorage.theme?(themename=localStorage.theme,z(themename)):z("theme-moleskine");
$(".appearance .item.theme").click(function(){themename=$(this).attr("id");$(this).removeClass("dim");z(themename)});localStorage.font?(fontname=localStorage.font,A(fontname)):A("font-mono");$(".appearance .font").click(function(){var a=$(this).attr("id");A(a)});localStorage.fontsize?(fontsize=localStorage.fontsize,B(fontsize)):B("fontsize-12");$(".appearance .fontsize").click(function(){var a=$(this).attr("id");B(a)});localStorage.tagPref=="no-tags"&&($("html body").addClass("no-tags"),$(".tag-pref").addClass("dim"));
$(".appearance .tag-pref").click(function(){$(this).hasClass("dim")?($(this).removeClass("dim"),$("html body").removeClass("no-tags"),localStorage.removeItem("tagPref")):($(this).addClass("dim"),$("html body").addClass("no-tags"),localStorage.setItem("tagPref","no-tags"))});$(".note .textarea textarea").live("focus",function(){$(".current .note-directions").addClass("show");$("div.current").addClass("highlight")});$(".note .textarea textarea").live("blur",function(){var a=$(this).parent().parent().attr("id"),
b=$("#list-"+a).data();$.inArray("markdown",b.systemtags)>-1&&p(a,"on");if($(this).val()!=b.content)blurnote=$.parseJSON(localStorage.getItem(a)),blurnote.content=$(this).val(),blurnote.modifydate=(new Date).getTime()/1E3,blurnote.syncnum+=1,localStorage.setItem(a,JSON.stringify(blurnote)),m(blurnote.key),g();$(".current .note-directions").removeClass("show");$(".current").removeClass("highlight")});$(".addnote").click(function(){F()});$(".tags").click(function(){x()});$(".reload").click(function(){g()});
$(".close").click(function(){k()});$(".delete").live("click",function(){$(this).parent().children(".message").toggleClass("show").html('Mark this note as deleted?  <a href="#" class="confirmDelete">Delete</a> / <a href="#" class="cancelDelete">Cancel</a>')});$(".confirmDelete").live("click",function(){var a=notekey=$(this).parent().parent().attr("id");note=$("#list-"+a).data();note.deleted=1;note.modifydate=(new Date).getTime()/1E3;note.syncnum+=1;localStorage.setItem(note.key,JSON.stringify(note));
$("#list-"+a).remove();$("#"+a).remove();$(".listnote:first").addClass(".selected");l();return!1});$(".cancelDelete").live("click",function(){$(".message").removeClass("show");return!1});$(".listnote").live("click",function(){$(".selected").removeClass("selected");$(this).addClass("selected");$(".current").removeClass("current");$("#"+$(this).attr("id").split("list-").pop()).addClass("current");o()});$(".note").live("click",function(){$(".current").removeClass("current");$(this).addClass("current");
$(".selected").removeClass("selected");$("#list-"+$(this).attr("id")).addClass("selected");o()});$(".textarea:not(a)").live("click",function(a){$(a.target).parents(".tag-area").length>0||(p($(this).parent().attr("id"),"off","temp"),$(this).children("textarea").focus())});$(".markdown a").live("click",function(){$(this).attr("target","_blank")});$(".settings_icon").click(function(){$(".settings").toggleClass("show")});$(".userdata").click(function(){$(".settings").toggleClass("show");$(".settings_pane").removeClass("show");
$(".settings_pane.data").addClass("show")});$(".settings .tabs a").click(function(a){$(".settings_pane").removeClass("show");$(".settings_pane."+$(this).text().replace(" ","_")).addClass("show");a.preventDefault();return!1});$(".versions-button").live("mousedown",function(){notekey=$(this).parent().attr("id");G(notekey)});$(".version-item").live("mousedown",function(){$(".diff-view").hasClass("depressed")?$(".versions-right .right-inner").html($(this).data().diff):$(".versions-right .right-inner").html('<textarea disabled="disabled">'+
...
$(".versions-right .right-inner").html('<textarea disabled="disabled">'+$(".version-select").data().content+"</textarea>")});$(".overlay").click(function(){k()});$(".versions-toolbar .revert-version").live("mousedown",function(){if($(".version-select").length==1)return notekey=$(".version-select").attr("id").split("version").shift(),verContent=$(".version-select").data().content,$(".selected").removeClass("selected"),$("#list-"+notekey).addClass("selected"),$(".current").removeClass("current"),$("#"+
notekey).addClass("current"),o(),k(),$(".note.current .textarea textarea").val(verContent).focus(),!1});$(".clear-tag-filter").live("mouseup",function(){H()});$(".labels .item").live("mouseup",function(){y($(this).attr("id"))});$(".tagit-label").live("mouseup",function(){y($(this).text())});$(".markdown-button").live("mouseup",function(){p($(this).parent().attr("id"))});$(".pin-button").live("mouseup",function(){E()});$(".cleardata").click(function(){localStorage.clear()});$(document).bind("keydown",
"esc",function(){k()});$(document).bind("keydown","j",function(){s()});$(document).bind("keydown","right",function(){s()});$(document).bind("keydown","down",function(){s()});$(document).bind("keydown","k",function(){t()});$(document).bind("keydown","left",function(){t()});$(document).bind("keydown","up",function(){t()});$(document).bind("keydown","return",function(){$(".current").hasClass("markdown-on")&&p($(".current").attr("id"),"off","temp");$(".note.current .textarea textarea").focus();return!1});
$(document).bind("keydown","tab",function(){$(".note.current .textarea textarea").focus();return!1});$(document).bind("keydown","shift+/",function(){$(".settings").toggleClass("show")});$(document).bind("keydown","/",function(){$(".search input").select();return!1});$(document).bind("keydown","c",function(){F();return!1});$(document).bind("keydown","s",function(){g();return!1});$(document).bind("keydown","p",function(){E($(".selected"))});$(document).bind("keydown","f",function(){r()});$(document).bind("keydown",
"l",function(){x();return!1});$(document).bind("keydown","t",function(){x();return!1});$(document).bind("keydown","v",function(){G($(".current").attr("id"));return!1});$(document).bind("keydown","a",function(){$("html body").removeClass("tag-filter");$(".active-tag-filter").remove();$(".listnote").removeClass("tag-hide");$(".selected").removeClass("selected");$(".listnote:not(.search-hide,.tag-hide):first").addClass("selected");$(".search input").val("");u();return!1});$(document).bind("keydown",
"m",function(){p();return!1});$(".search input").keyup($.debounce(250,u));$(".search input").bind("keydown","return",function(){$(this).blur();return!1});$(".search input").bind("keydown","esc",function(){$(this).val("");u();$(this).blur();return!1});$(".search input").bind("keydown","down",function(){$(this).blur();s();return!1});$(".search input").bind("keydown","up",function(){$(this).blur();t();return!1});$(".labels input").keyup(function(a){a=a.which;a!=38&&a!=40&&a!=13&&S()});$(".labels input").bind("keydown",
"esc",function(){$(this).val("");$(this).blur();k();return!1});$(".labels input").bind("keydown","down",function(){nextOne=$(".label-select").nextAll(".item:not(.hide):first");nextOne.length>0&&($(".label-select").removeClass("label-select"),nextOne.addClass("label-select"));return!1});$(".labels input").bind("keydown","up",function(){prevOne=$(".label-select").prevAll(".item:not(.hide):first");prevOne.length>0&&($(".label-select").removeClass("label-select"),prevOne.addClass("label-select"));return!1});
$(".labels input").bind("keydown","return",function(){y($(".label-select .name").text());return!1});w(".note .textarea textarea")};$(function(){U();if(localStorage.email){$(".username").text(localStorage.email);if(localStorage.index){n(10);if(localStorage.index){index=$.parseJSON(localStorage.index);for(i=0;i<=index.data.length-1;i++)localStorage.getItem(index.data[i].key)!=null&&m(index.data[i].key)}$(".listnote").length>0&&(q(),l())}(new Date).getTime();g()}else window.location="/"})}();